...
class Map extends Component {
    ...
    componentDidMount () {
        console.log("mount")
        this.setState({display : this.props.display, updated : !this.state.updated})
    }
    ...
    componentDidUpdate (prevProps, prevState) {
        if(JSON.stringify(prevProps.display) !== JSON.stringify(this.props.display)){
    ...
    for (let g in updatedGraph){
        ...
        if (Number(g) === Number(roomId)){
            ...
            temp = { id: g, label: `${g}`, x: this.props.coor[g][0], y: this.props.coor[g][1], size: 15, color: "red", borderColor: this.getRandomColor(), type : "diamond"}
        } else {
            temp = { id: g, label: `${g}`, x: this.props.coor[g][0], y: this.props.coor[g][1], size: 1, color: "#282D31", borderColor: this.getRandomColor(), type : "equilateral"}
        }
        nodes.push(temp)
    }
        ...
        if (this.props.initialize && this.state.updated){
            ...
            <div className = "changeCanvas">
                ...
                <div className = "mapping">  
                    <Sigma  renderer = "canvas"  style = {{ maxWidth: "inherit", height: "800px"}}settings = {{drawEdges: true, clone: true}} graph ={display}>
